...
def classificar_eventos(
    ...
    count_from_work_start: bool = False,
    work_start_for_count: time | None = None,
):
    base = pd.DataFrame(
        {
    ...
    # Tipo canônico principal da situação crítica (inicialmente vazio)
    base["tipo_canon_situacao"] = None
    ...
    for idx, row in base.iterrows():
        ...
        if sum_in_window[ent] >= threshold:
            # Se for para contar somente a partir do início do expediente,
            # não dispara N1/N2/N3 antes do horário configurado
            if count_from_work_start and work_start_for_count is not None:
                if t.time() < work_start_for_count:
                    # Mantém janela/pontuação, mas não marca como crítica ainda
                    continue
            ...
            # Detalhes: todos os eventos da janela
            window_events = list(deques[ent])
            detalhes = []
            for t_ev, w_ev, tipo_ev, placa_ev in window_events:
            ...
            # Tipo principal da situação crítica: evento de maior peso na janela
            tipo_principal = None
            if window_events:
                # pega evento com maior peso
                max_ev = max(window_events, key=lambda x: x[1])
                tipo_principal = max_ev[2]  # tipo_ev
            # fallback: tipo da própria linha, se algo der errado
            if not tipo_principal:
                tipo_principal = tipo
            base.at[idx, "tipo_canon_situacao"] = tipo_principal

            # Limpa janela para não ficar acumulando
            deques[ent].clear()
            sum_in_window[ent] = 0
...
def tabela_tratativas(
    ...
    # Eventos que compõem a situação crítica (multi-linha)
    ...
    # Ação recomendada: agora usando tipo_canon_situacao (tipo principal da janela)
    merged["Ação Recomendada"] = merged.apply(
        lambda r: get_recommended_action(
            r.get("tipo_canon_situacao") or r.get("tipo_canon"),
            r.get("Nível Recomendado"),
        ),
        axis=1,
    )
        ...
        if col_datahora is None and (not col_data or not col_hora):
            ...
            with st.spinner("Processando..."):
                resultados, resumo = classificar_eventos(
                    ...
                    count_from_work_start=bool(count_from_shift_start),
                    work_start_for_count=work_start,
                )
            ...
            if trat.empty:
                ...
                # Tabela principal: só o essencial
                trat_display = trat[
                    [
                        "Motorista",
                        "Data/Hora Último Alerta",
                ...
                # "Clique" indireto: seleção de uma situação crítica para ver detalhes
                st.markdown("### Detalhamento da Situação Crítica selecionada")

                # usamos o índice do dataframe como opção
                idx_options = list(trat.index)
                if idx_options:
                    def _format_opt(idx):
                        row = trat.loc[idx]
                        return (
                            f"{row['Motorista']} - {row['Data/Hora Último Alerta']} "
                            f"({row['Nível Recomendado']}, {row['Status']})"
                        )

                    selected_idx = st.selectbox(
                        "Selecione uma situação crítica para ver detalhes",
                        options=idx_options,
                        format_func=_format_opt,
                    )

                    sel = trat.loc[selected_idx]
                    st.markdown(f"**Motorista:** {sel['Motorista']}")
                    st.markdown(f"**Data/Hora Último Alerta:** {sel['Data/Hora Último Alerta']}")
                    st.markdown(f"**Status:** {sel['Status']}")
                    st.markdown(f"**Nível Recomendado:** {sel['Nível Recomendado']}")
                    st.markdown(
                        "**Dentro do expediente?** "
                        + ("Sim" if sel["Dentro do Expediente?"] else "Não")
                    )

                    st.markdown("**Eventos da Situação Crítica:**")
                    st.markdown(
                        sel["Eventos da Situação Crítica"].replace("\n", "  \n")
                    )

                    st.markdown("**Ação Recomendada:**")
                    if sel["Ação Recomendada"].strip():
                        st.markdown(
                            sel["Ação Recomendada"].replace("\n", "  \n")
                        )
                    else:
                        st.markdown("_(Nenhuma ação configurada para esse tipo de evento)_")
            ...
            st.download_button(
                "Baixar Excel",
                data=buf.getvalue(),
                file_name="classificacao_janela_critica_final16.xlsx",
