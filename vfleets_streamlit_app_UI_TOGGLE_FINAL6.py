...
EXCLUDE_ROTULOS = {"Falso Positivo","Uso de celular","Diversos Painel/Retrovisor/Comendoo"}
...
def tipo_to_canon(tipo: str) -> str | None:
    ...
    if not t:
        return None
    if "BOCEJO" in t: return "BOCEJO"
    if "FADIGA" in t: return "FADIGA_DO_MOTORISTA"
    if "SEM" in t and "CINTO" in t: return "SEM_CINTO"
    if ("CELULAR" in t) or ("MANUSEIO" in t and "CEL" in t): return "USO_DE_CELULAR"
    if ("EXCESSO" in t) or ("VELOCIDADE" in t): return "EXCESSO_VELOCIDADE"
    if "POSSIVEL" in t and "COLISAO" in t: return "POSSIVEL_COLISAO"
    if "ULTRAPASSAGEM" in t and "ILEGAL" in t: return "ULTRAPASSAGEM_ILEGAL"
    if "CAMERA" in t and ("OBSTRUIDA" in t or "OBSTRU" in t): return "CAMERA_OBSTRUIDA"
    return t
...
def entidade_simplificada(ent: str) -> str:
    if not isinstance(ent, str): return ent
    if ent.startswith("MOTORISTA::"): return ent.replace("MOTORISTA::","Motorista: ")
    if ent.startswith("PLACA::"): return ent.replace("PLACA::","Placa: ")
    return ent
...
def classificar_eventos(
    ...
    base["rotulo"] = (df[col_rotulo].astype(str).str.strip()
        if col_rotulo and col_rotulo in df.columns else "")
    ...
    # Canonização do tipo base
    base["tipo_canon"] = base["tipo_raw"].map(tipo_to_canon)

    # Especialização por nível de excesso de velocidade (antes de calcular pesos)
    if isinstance(base["nivel_evento"], pd.Series):
        mask_vel = base["tipo_canon"].eq("EXCESSO_VELOCIDADE")
        mask_baixo = mask_vel & base["nivel_evento"].isin(["BAIXO"])  # até 20%
        mask_medio = mask_vel & base["nivel_evento"].isin(["MEDIO","MÉDIO"])  # 20–30%
        mask_alto = mask_vel & base["nivel_evento"].isin(["ALTO"])  # acima de 30%
    ...
    # Entidade (preferência por motorista, fallback para placa quando "Sem Identificação")
    ...
    # Peso por tipo
    base["peso"] = base["tipo_canon"].apply(lambda c: int(DEFAULT_PESOS.get(c, 0)) if c else 0)

    # Excluir por rótulo específico
    if isinstance(base["rotulo"], pd.Series):
        base.loc[base["rotulo"].isin(EXCLUDE_ROTULOS), "peso"] = 0

    # Ordenação temporal por entidade
    ...
    for idx, row in base.iterrows():
        ...
        if sum_in_window[ent] >= threshold:
            ...
            for t_ev, w_ev, tipo_ev, placa_ev in list(deques[ent]):
                ts = pd.to_datetime(t_ev).strftime("%H:%M:%S")
...
def ranking_criticos(df_resultados: pd.DataFrame):
    ...
    if crit.empty:
        return pd.DataFrame(columns=["Entidade","Tipo de Entidade","Último Nível","Data/Hora"])
    map_val = {"N1":1,"N2":2,"N3":3}
    crit["nivel_val"] = crit["nivel"].map(map_val).fillna(0).astype(int)
    crit = crit.sort_values(["nivel_val","data_hora"], ascending=[False,False]).copy()
    best = crit.drop_duplicates(subset=["entidade"], keep="first").copy()
    best["Tipo de Entidade"] = np.where(best["entidade"].str.startswith("MOTORISTA::"), "Motorista","Placa")
...
def _parse_datetime_from_raw(raw: pd.DataFrame, col_datahora: str | None, col_data: str | None, col_hora: str | None) -> pd.Series:
    if col_datahora and col_datahora in raw.columns and col_datahora != "":
        return pd.to_datetime(raw[col_datahora], errors="coerce", dayfirst=True, infer_datetime_format=True)
    else:
        data = raw[col_data].astype(str).str.strip() if col_data and col_data in raw.columns else ""
        hora = raw[col_hora].astype(str).str.strip() if col_hora and col_hora in raw.columns else ""
        return pd.to_datetime(data + " " + hora, errors="coerce", dayfirst=True, infer_datetime_format=True)
...
def tabela_tratativas(resultados: pd.DataFrame, raw: pd.DataFrame,
                      col_motorista_raw: str | None, col_datahora: str | None, col_data: str | None, col_hora: str | None,
                      work_start: time | None, work_end: time | None, only_last: bool = True) -> pd.DataFrame:
    # Descobrir coluna STATUS (W/X com nome STATUS)
    status_col = None
    ...
    # Manter apenas motoristas identificados (linhas com entidade MOTORISTA::)
    crit = crit[crit["entidade"].astype(str).str.startswith("MOTORISTA::")].copy()
    if crit.empty:
        return pd.DataFrame()

    # Extrair o nome do motorista da entidade
    crit["motorista_nome"] = crit["entidade"].astype(str).str.replace("^MOTORISTA::", "", regex=True)

    # Preparar raw com datetime e motorista
    ...
    if only_last:
        # Último alerta crítico por motorista
        crit_last = crit.sort_values("data_hora", ascending=False).drop_duplicates(subset=["motorista_nome"], keep="first").copy()
        # Juntar pelo motorista + timestamp exato (último evento que disparou a crítica)
        ...
        # Todas as situações críticas (uma linha por ocorrência crítica por motorista)
        crit_all = crit.copy()
    ...
    # Traduzir / calcular campos
    merged["Status"] = merged[status_col].apply(_traduz_status)
    merged["Motorista"] = merged["motorista_nome"]
    merged["Data/Hora Último Alerta"] = merged["data_hora"]  # ainda datetime aqui, pra avaliar janela
    merged["Nível Recomendado"] = merged["nivel"]

    # Determinar prioridade por janela de trabalho
    if work_start is None: work_start = time(8, 0)
    if work_end is None: work_end = time(17, 0)
    ...
    merged["Data/Hora Último Alerta"] = format_datetime_series(pd.to_datetime(merged["Data/Hora Último Alerta"]))
    merged["Prioridade"] = np.where(
        (merged["Status"] == "Pendente Tratativa") & (merged["Dentro do Expediente?" ] == True),
        1, 2
    )

    out = merged[["Motorista","Data/Hora Último Alerta","Status","Nível Recomendado","Dentro do Expediente?","Prioridade"]]\
...
with st.sidebar:
    ...
    prefer_motorista = st.checkbox("Preferir Motorista (fallback para Placa quando 'Sem Identificação')", value=True)
    ...
    # Horário de trabalho configurável (default 08:00 a 17:00)
    st.markdown("**Horário de trabalho do analista**")
    work_start = st.time_input("Início do expediente", value=time(8, 0), key="work_start")
    work_end = st.time_input("Fim do expediente", value=time(17, 0), key="work_end")

    # Toggle para exibir apenas a última situação crítica por motorista ou todas
    only_last = st.checkbox("Mostrar apenas a última situação crítica por motorista", value=False, help="Desmarque para ver todas as situações críticas de cada motorista.")
    ...
    st.dataframe(pesos_df.sort_values("Tipo"), use_container_width=True, height=360)
...
if up is not None:
    ...
    try:
        ...
        raw = pd.read_csv(io.BytesIO(content), sep=None, engine="python", encoding="latin1")
    ...
    # Filtro de Motorista
    motoristas_unicos = (raw[col_motorista].dropna().astype(str).str.strip().unique().tolist()
                         if col_motorista in raw.columns else [])
    motoristas_unicos = [m for m in motoristas_unicos if m and m != "Sem Identificação"]
    motoristas_unicos.sort()
    sel_mot = st.selectbox("Filtrar por Motorista (opcional)", options=["(Todos)"] + motoristas_unicos, index=0)

    raw_filtered = raw.copy()
    if sel_mot != "(Todos)":
        raw_filtered = raw_filtered[raw_filtered[col_motorista].astype(str).str.strip() == sel_mot].copy()
    ...
    if not col_tipo or not col_placa or not col_motorista:
        ...
        if col_datahora is None and (not col_data or not col_hora):
            ...
            # >>> NOVA TABELA: Tratativas por motorista, com prioridade por expediente
            st.subheader("Tratativas por motorista" + (" (apenas última por motorista)" if only_last else " (todas as situações críticas)"))
            trat = tabela_tratativas(resultados, raw_filtered, col_motorista, col_datahora, col_data, col_hora,
                                     work_start=work_start, work_end=work_end, only_last=only_last)
            if trat.empty:
                st.info("Sem situações críticas de motoristas ou coluna STATUS ausente no CSV.")
            else:
                pendentes_exp = trat[(trat["Status"] == "Pendente Tratativa") & (trat["Dentro do Expediente?"] == True)]
                colm1, colm2, colm3 = st.columns(3)
                with colm1:
                    st.metric("Pendentes dentro do expediente", int(pendentes_exp.shape[0]))
                with colm2:
                    st.metric("Pendentes (total)", int(trat[trat["Status"] == "Pendente Tratativa"].shape[0]))
                with colm3:
                    st.metric("Já tratados", int(trat[trat["Status"] == "Já tratado"].shape[0]))

                # Exibir com os prioritários no topo
                st.dataframe(trat.drop(columns=["Prioridade"]), use_container_width=True, height=380)
            ...
            with colA:
                ...
                st.dataframe(rank[rank["Tipo de Entidade"]=="Motorista"], use_container_width=True, height=340)
            with colB:
                st.caption("Placas")
                st.dataframe(rank[rank["Tipo de Entidade"]=="Placa"], use_container_width=True, height=340)
            ...
            st.download_button("Baixar Excel", data=buf.getvalue(),
                               file_name="classificacao_janela_critica_final12.py.xlsx",
