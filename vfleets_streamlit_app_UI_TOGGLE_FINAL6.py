...
def tabela_tratativas(resultados: pd.DataFrame, raw: pd.DataFrame,
                      ...
                      work_start: time | None, work_end: time | None) -> pd.DataFrame:
    ...
    merged["Status"] = merged[status_col].apply(traduz_status)
...
if up is not None:
    ...
    if not col_tipo or not col_placa or not col_motorista:
        ...
        if col_datahora is None and (not col_data or not col_hora):
            ...
            st.subheader("Tratativas por motorista (última situação crítica)")
            ...
            st.download_button("Baixar Excel", data=buf.getvalue(),
                               file_name="classificacao_janela_critica_final11.xlsx",
